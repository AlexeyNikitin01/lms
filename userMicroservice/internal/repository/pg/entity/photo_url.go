...
func (o *PhotoURL) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(photoURLAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(photoURLPrimaryKeyColumns) == 0 {
				return errors.New("entity: unable to upsert photo_url, could not build conflict column list")
			}

			conflict = make([]string, len(photoURLPrimaryKeyColumns))
			copy(conflict, photoURLPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"photo_url\"", updateOnConflict, ret, update, conflict, insert, opts...)
