...
var VideoLectureTableColumns = struct {
	...
	CreatedAt: "video_lectures.created_at",
	UpdatedAt: "video_lectures.updated_at",
	DeletedAt: "video_lectures.deleted_at",
}
...
var VideoLectureWhere = struct {
	...
	CreatedAt: whereHelpertime_Time{field: "\"video_lectures\".\"created_at\""},
	UpdatedAt: whereHelpertime_Time{field: "\"video_lectures\".\"updated_at\""},
	DeletedAt: whereHelpernull_Time{field: "\"video_lectures\".\"deleted_at\""},
}
...
var (
	videoLectureAllColumns            = []string{"id", "course_id", "url", "created_at", "updated_at", "deleted_at"}
	videoLectureColumnsWithoutDefault = []string{"course_id", "url"}
	videoLectureColumnsWithDefault    = []string{"id", "created_at", "updated_at", "deleted_at"}
...
func VideoLectures(mods ...qm.QueryMod) videoLectureQuery {
	mods = append(mods, qm.From("\"video_lectures\""), qmhelper.WhereIsNull("\"video_lectures\".\"deleted_at\""))
	q := NewQuery(mods...)
...
func FindVideoLecture(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*VideoLecture, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"video_lectures\" where \"id\"=$1 and \"deleted_at\" is null", sel,
	)
...
func (o *VideoLecture) Delete(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if o == nil {
	...
	if hardDelete {
		...
		sql = fmt.Sprintf("UPDATE \"video_lectures\" SET %s WHERE \"id\"=$2",
			strmangle.SetParamNames("\"", "\"", 1, wl),
		)
		valueMapping, err := queries.BindMapping(videoLectureType, videoLectureMapping, append(wl, videoLecturePrimaryKeyColumns...))
		if err != nil {
			return 0, err
		}
...
func (q videoLectureQuery) DeleteAll(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if q.Query == nil {
...
func (o VideoLectureSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor, hardDelete bool) (int64, error) {
	if len(o) == 0 {
		return 0, nil
	}
	...
	if hardDelete {
		...
		wl := []string{"deleted_at"}
		sql = fmt.Sprintf("UPDATE \"video_lectures\" SET %s WHERE "+
			strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 2, videoLecturePrimaryKeyColumns, len(o)),
			strmangle.SetParamNames("\"", "\"", 1, wl),
		)
...
func VideoLectureExists(ctx context.Context, exec boil.ContextExecutor, iD int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"video_lectures\" where \"id\"=$1 and \"deleted_at\" is null limit 1)"
